...
export async function saveEndpointsJson(entrypoints, result, rootDir, outputDir, publicDir = outputDir, extras = {}) {
    ...
    entrypoints.forEach((entrypoint) => {
        ...
        switch (loader) {
            case 'css': {
                let list = entrypointsJson[basename]['css'] = entrypointsJson[basename]['css'] || [];
    ...
    await writeFile(path.join(outputDir, 'entrypoints.json'), JSON.stringify({ entrypoints: entrypointsJson, ...extras }, null, 2));
}
