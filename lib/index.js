...
export async function build(options) {
    ...
    let definitions = {};
    if (platform === 'browser') {
        definitions['process.env.NODE_ENV'] = JSON.stringify(process.env.NODE_ENV || minify ? 'production' : 'development');
    ...
    return await esbuild.build({
        ...
        watch,
        plugins: [
            (await import('./plugins/postcss')).postcssPlugin(),
            (await import('./plugins/url')).urlPlugin(loaders),
        ],
    });
}
