...
export function collectScripts(
    ...
    base,
    outdir,
    targets = { scriptsTarget: "es6", modulesTarget: "es2020" },
    options
) {
    return [
        ...collect($, dom, 'script[src][type="module"]', "src").map(
            (element) => ({
                options: {
                    entryPoints: [
                        path.resolve(
        ...
        ...dom
            .find('script[type="module"]:not([src])')
            .get()
            .map((element) => {
        ...
        ...dom
            .find(
                'script:not([src]):not([type]), script[type="text/javascript"]:not([src]), script[type="application/javascript"]:not([src])'
            )
            .get()
            .map((element) => {
                const code = /** @type {string} */ ($(element).html());
                ...
                return {
                    ...
                    options: {
                        ...
                        target: targets.scriptsTarget,
                        format: /** @type {import('esbuild').Format} */ (
                            "iife"
                        ),
                        globalName: undefined,
                        entryNames: `iife/${options.entryNames || "[name]"}`,
                        chunkNames: `iife/${options.chunkNames || "[name]"}`,
                        assetNames: `iife/assets/${
                            options.assetNames || "[name]"
                        }`,
                        splitting: false,
                    },
