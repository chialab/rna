...
`);
    ...
    it('should load a classic worker without bundle', async () => {
        ...
        expect(result.text).to.be.equal(`const worker = new Worker(new URL("./worker.js?emit=chunk", import.meta.url).href);
export {
  worker
};
`);
...
`);
    ...
    it('should load a module worker without bundle', async () => {
        ...
        expect(result.text).to.be.equal(`const worker = new Worker(new URL("./worker.js?emit=chunk", import.meta.url).href, { type: "module" });
export {
  worker
};
`);
